...
export class ShiftModel {
   ...
   * Uses available_shifts_view which includes business info and booking counts
   */
  static async findAvailable(): Promise<any[]> {
    const query = `
      SELECT *
      FROM available_shifts_view
      WHERE start_time > NOW()
      AND status = 'open'
      AND (
        max_workers IS NULL 
        OR available_spots > 0
      )
