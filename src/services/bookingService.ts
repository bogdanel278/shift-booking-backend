...
export class BookingService {
  ...
  static async createBooking(input: CreateBookingInput): Promise<Booking> {
    ...
    // CRITICAL: Worker must have at least one verified right-to-work record
    ...
    // Validate current capacity before creating booking
    const hasCapacity = await ShiftModel.hasCapacity(input.shift_id);
    if (!hasCapacity) {
